...
namespace leaktracer {
    ...
    MemoryTrace::MemoryTrace(void) :
            __setupDone(false), __monitoringAllThreads(false), __monitoringReleases(false),
            __monitoringDisabler(0) {
    }
    ...
    MemoryTrace::init_full() {
        ...
        if (getenv("LEAKTRACER_ONSTART_STARTALLTHREAD") ||
            getenv("LEAKTRACER_AUTO_REPORTFILENAME")) {
            leaktracer::MemoryTrace::GetInstance().startMonitoringAllThreads();
        }
#ifdef USE_BACKTRACE
    ...
    void MemoryTrace::MemoryTraceOnExit(void) {
        if (getenv("LEAKTRACER_ONEXIT_REPORT") || getenv("LEAKTRACER_AUTO_REPORTFILENAME")) {
            ...
            if (!(reportName = getenv("LEAKTRACER_ONEXIT_REPORTFILENAME")) &&
                !(reportName = getenv("LEAKTRACER_AUTO_REPORTFILENAME"))) {
...
// cleanup per-thread data
    void MemoryTrace::CleanUpThreadData(void *ptrThreadOptions) {
        if (ptrThreadOptions != NULL)
            GetInstance().removeThreadOptions(
                    reinterpret_cast<ThreadMonitoringOptions *>(ptrThreadOptions));
    }
    ...
    _Unwind_Reason_Code Unwind_Trace_Fn(_Unwind_Context *context, void *hnd) {
        ...
        if (traceHanle->pos != ALLOCATION_STACK_DEPTH) {
            traceHanle->backtrace[traceHanle->pos] = (void *) (ip -
                                                               (_Unwind_Word) s_P2pSODlInfo.dli_fbase);
            ++traceHanle->pos;
...
// writes all memory leaks to given stream
    void MemoryTrace::writeLeaksPrivate(std::ostream &out) {
        ...
        while (__allocations.getNextPair(&info, &p)) {
            ...
            out << "leak, ";
            out << "time=" << std::fixed << std::right << std::setprecision(precision)
                << std::setfill('0') << std::setw(maxsecwidth + 1 + precision) << d
                << ", "; // setw(16) ?
            out << "stack=";
